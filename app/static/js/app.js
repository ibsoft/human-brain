...
if(document.getElementById("activityChart")){
  ...
  const requestTokenOptions={...chartOptions,scales:{...chartOptions.scales,x:{type:"linear",min:0,ticks:{color:"#98b9aa"},grid:{color:"rgba(128,230,178,.12)"}},y:{...chartOptions.scales.y,title:{display:true,text:"tokens",color:"#98b9aa"}}}};
  ...
  const spikePlugin={id:"requestSpikes",afterDatasetsDraw(chart){if(!chart.canvas.dataset.spikeChart) return;const {ctx,scales:{y}}=chart;const base=y.getPixelForValue(0);chart.data.datasets.forEach((dataset,datasetIndex)=>{const meta=chart.getDatasetMeta(datasetIndex);ctx.save();ctx.strokeStyle=dataset.borderColor;ctx.fillStyle=dataset.borderColor;ctx.lineWidth=2;meta.data.forEach(point=>{ctx.beginPath();ctx.moveTo(point.x,base);ctx.lineTo(point.x,point.y);ctx.stroke();ctx.beginPath();ctx.arc(point.x,point.y,2.4,0,Math.PI*2);ctx.fill();});ctx.restore();});}};
  ...
  const buildSpikeChart=(id,key,options=chartOptions,unit="ms")=>{
    const target=document.getElementById(id);
    if(!target) return;
    target.dataset.spikeChart="1";
    const data=dataset(key);
    ...
    const formatMinute=value=>{
      if(windowStart && !Number.isNaN(windowStart.getTime())){
        const stamp=new Date(windowStart.getTime()+(Math.round(Number(value)||0)*60000));
        if(Number(data.x_max||1440)>43200) return stamp.toLocaleDateString(undefined,{month:"short",year:"2-digit"});
        if(Number(data.x_max||1440)>2880) return stamp.toLocaleDateString(undefined,{month:"short",day:"numeric"});
        return stamp.toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit",hour12:false});
      }
      const total=(startMinute+Math.round(Number(value)||0)+1440*10)%1440;
      return `${String(Math.floor(total/60)).padStart(2,"0")}:${String(total%60).padStart(2,"0")}`;
    };
    const spikeOptions={...options,scales:{...options.scales,x:{...options.scales.x,max:Number(data.x_max||1440),ticks:{...options.scales.x.ticks,callback:formatMinute}}},plugins:{...options.plugins,tooltip:{callbacks:{title:items=>items.length?formatMinute(items[0].parsed.x):"",label:item=>`${item.dataset.label}: ${item.parsed.y} ${unit}`}}}};
  ...
  buildSpikeChart("agentRequestSpeedChart","agent_request_speed",requestSpeedOptions,"ms");
  buildSpikeChart("agentRequestTokensChart","agent_request_tokens",requestTokenOptions,"tokens");
